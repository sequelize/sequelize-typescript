...
describe('instance', () => {
        ...
        User
          ...
          .then((user) =>
            sequelize
              ...
              .then(transaction =>
                user
                  .decrement('number', {by: 2, transaction})
                  .then(() =>
                    User
                      .findAll()
                      .then((users1) =>
  ...
  describe('default values', () => {
    ...
    describe('allowNull date', () => {
      it('should be just "null" and not Date with Invalid Date', () => {
        ...
        User.build({username: 'a user'}).save().then(() => {
      ...
      it('should be the same valid date when saving the date', () => {
        ...
        var date = new Date();
        User.build({username: 'a user', dateAllowNullTrue: date}).save().then(() => {
          User.findOne({where: {username: 'a user'}}).then((user) => {
  ...
  describe('complete', () => {
    it('gets triggered if an error occurs', () => {
      User.findOne({where: ['asdasdasd']}).catch((err) => {
        expect(err).to.exist;
        expect(err.message).to.exist;
      });
    });
  ...
  describe('save', () => {
    ...
    it('handles an entry with primaryKey of zero', () => {
      ...
      user2.sync().then(function () {
        user2.create({id: 0, username: username}).then(function (user) {
          expect(user).to.be.ok;
          expect(user.id).to.equal(0);
          expect(user.username).to.equal(username);
          user2.findById(0).then(function (user) {
            expect(user).to.be.ok;
            expect(user.id).to.equal(0);
            expect(user.username).to.equal(username);
