...
describe('instance', () => {
  ...
  describe('instanceof', () => {
    ...
    it('should return true for include values of found instance', () =>
      Book.findOne({ include: [Page] }).then((book) => {
  ...
  describe('isNewRecord', () => {
    ...
    it('returns false for created objects', () =>
      User.create({ username: 'user' }).then((user) => {
        expect(user.id).to.not.be.null;
        expect(user.isNewRecord).to.not.be.ok;
      }));

    it('returns false for objects found by find method', () =>
      User.create({ username: 'user' }).then(() =>
        User.create({ username: 'user' }).then((user) =>
  ...
  describe('increment', () => {
    ...
    it('with array', () =>
      User.findByPk(1).then((user1) =>
        user1.increment(['aNumber'], { by: 2 }).then(() =>
          User.findByPk(1).then((user3) => {
    ...
    it('with single field and no value', () =>
      User.findByPk(1).then((user1) =>
        user1.increment('aNumber').then(() =>
          User.findByPk(1).then((user2) => {
    ...
    it('with key value pair', () =>
      User.findByPk(1).then((user1) =>
        user1.increment({ aNumber: 1, bNumber: 2 }).then(() =>
          User.findByPk(1).then((user3) => {
    ...
    it('with timestamps set to true', () => {
      ...
      return TimeStampsUser.sync({ force: true })
  ...
  describe('decrement', () => {
    ...
    it('with array', () =>
      User.create({ aNumber: 0 })
        .then(() => User.findByPk(1))
        .then((user1) =>
          user1.decrement(['aNumber'], { by: 2 }).then(() =>
            User.findByPk(1).then((user3) => {
              expect(user3.aNumber).to.be.equal(-2);
            })
          )
    ...
    it('with key value pair', () =>
      User.create({ aNumber: 0, bNumber: 0 })
        .then(() => User.findByPk(1))
        .then((user1) =>
          user1.decrement({ aNumber: 1, bNumber: 2 }).then(() =>
            User.findByPk(1).then((user3) => {
              expect(user3.aNumber).to.be.equal(-1);
              expect(user3.bNumber).to.be.equal(-2);
            })
  ...
  describe('reload', () => {
    ...
    it('should return a reference to the same DAO instead of creating a new one', () =>
      User.create({ username: 'John Doe' }).then((originalUser) =>
        originalUser.update({ username: 'Doe John' }).then(() =>
          originalUser.reload().then((updatedUser) => {
    ...
    it('should update the values on all references to the DAO', () =>
      User.create({ username: 'John Doe' }).then((originalUser) =>
        User.findByPk(originalUser.id).then((updater) =>
          updater.update({ username: 'Doe John' }).then(() => {
    ...
    it('should support updating a subset of attributes', () =>
      User.create({ aNumber: 1, bNumber: 1 })
    ...
    it('should update the associations as well', () =>
      Book.sync({ force: true })
        ...
        .then((book) =>
          Page.create({ content: 'om nom nom' }).then((page) =>
            book
              .$set('pages', [page])
              .then(() => Book.findOne({ where: { id: book.id }, include: [Page] }))
              .then((leBook: Book) =>
                page.update({ content: 'something totally different' }).then((_page: Page) => {
                  ...
                  return leBook.reload().then((_leBook: any) => {
        ...
        .then((book) =>
          Page.create().then((page) =>
            book['setPages']([page]) // todo
              .then(() =>
                Book.findOne({ where: { id: book.id } }).then((leBook) => {
                  return leBook.reload({ include: [Page] }).then((_leBook: Book) => {
                    expect(_leBook.pages.length).to.equal(1);
  ...
  describe('default values', () => {
    ...
    describe('allowNull date', () => {
      it('should be just "null" and not Date with Invalid Date', () =>
        User.build({ username: 'a user' })
          .save()
          .then(() =>
            User.findOne({ where: { username: 'a user' } }).then((user) => {
      ...
      it('should be the same valid date when saving the date', () => {
        const date = new Date();
        return User.build({ username: 'a user', dateAllowNullTrue: date })
          .save()
          .then(() =>
            User.findOne({ where: { username: 'a user' } }).then((user) => {
  ...
  describe('complete', () => {
    it('gets triggered if an error occurs', () =>
      User.findOne({ where: ['asdasdasd'] as any }).catch((err) => {
        expect(err).to.exist;
        expect(err.message).to.exist;
      }));
  ...
  describe('save', () => {
    ...
    describe('hooks', () => {
      ...
      it('should validate attributes added in hooks when default fields are used', () => {
        ...
        return UserWithValidation.sync({ force: true })
          .then(() =>
            UserWithValidation.create({
              name: 'A',
              bio: 'A',
    ...
    it('does not update timestamps when passing silent=true', () => {
      ...
      return TimeStampsUser.create({ username: 'user' }).then((user) => {
        const updatedAt = user.updatedAt;

        clock.tick(1000);

        return user.update({ username: 'userman' }, { silent: true }).then(() => {
    ...
    it('does not update timestamps when passing silent=true in a bulk update', () => {
      ...
      return TimeStampsUser.bulkCreate(data)
    ...
    describe('when nothing changed', () => {
      ...
      it('does not update timestamps', () =>
        TimeStampsUser.create({ username: 'John' })
          ...
          .then((user) => {
            ...
            return user.save().then((newlySavedUser) => {
              ...
              return TimeStampsUser.findOne({ where: { username: 'John' } }).then(
                (_newlySavedUser) => {
    ...
    describe('with custom timestamp options', () => {
      it('updates the createdAt column if updatedAt is disabled', () => {
        ...
        UserWithCreatedAtButWithoutUpdatedAt.sync()
          .then(() => UserWithCreatedAtButWithoutUpdatedAt.create({ name: 'john doe' }))
          .then((johnDoe) => {
    ...
    it('should fail a validation upon creating', () =>
      User.create({ aNumber: 'hello' as any }).catch((err) => {
        expect(err).to.exist;
    ...
    it('should fail a validation upon creating with hooks false', () =>
      User.create({ aNumber: 'hello' as any }, { hooks: false } as any).catch((err) => {
        expect(err).to.exist;
        expect(err).to.be.instanceof(Object);
        expect(err.get('aNumber')).to.be.instanceof(Array);
        expect(err.get('aNumber')[0]).to.exist;
    ...
    it('should fail a validation upon building', () =>
      User.build({ aNumber: 0, username: 'aaaaaaaaaaaaaaaaaaaaaaaaaa' })
        .save()
        .catch((err) => {
          expect(err).to.exist;
          expect(err).to.be.instanceof(Object);
    ...
    describe('eagerly loaded objects', () => {
      ...
      beforeEach(() =>
        UserEager.sync({ force: true }).then(() => ProjectEager.sync({ force: true }))
      );

      it('saves one object that has a collection of eagerly loaded objects', () =>
        UserEager.create({ username: 'joe', age: 1 }).then((user) =>
          ProjectEager.create({ title: 'project-joe1', overdueDays: 0 }).then((project1) =>
            ProjectEager.create({ title: 'project-joe2', overdueDays: 0 })
      ...
      it('saves many objects that each a have collection of eagerly loaded objects', () =>
        Promise.all([
          // Converting to native es6 promises
          ...
          Promise.all([
            ...
            .then((simpsons) => {
              ...
              return Promise.all([_bart.save(), _lisa.save()]);
            })
      ...
      it('saves many objects that each has one eagerly loaded object (to which they belong)', () =>
        Promise.all([
          ...
          .then((projects) => {
            ...
            return Promise.all([projects[0].save(), projects[1].save()]);
          })
          .then(() =>
            ProjectEager.findAll({
  ...
  describe('toJSON', () => {
    ...
    it("dont return instance that isn't defined", () =>
      NiceProject.create({ user: null })
        .then((project) =>
          NiceProject.findOne({
            where: {
              id: project.id,
            },
            include: [{ model: NiceUser, as: 'user' }],
          })
        )
        .then((project) => {
  ...
  describe('findAll', () => {
    ...
    it('escapes a single single quotes properly in where clauses', () =>
      User.create({ username: "user'name" })
  ...
  describe('restore', () => {
    it('returns an error if the model is not paranoid', () =>
      User.create({ username: 'Peter' })
        ...
        .catch((error) => expect(error.message).to.equal('Model is not paranoid'))); // Native Promises
    ...
    it('restores a previously deleted model', () => {
      ...
      return ParanoidUser2.sync({ force: true })
        .then(() => {
